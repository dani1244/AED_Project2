...
Graph* GraphCreateTranspose(const Graph* g) {
  ...
  // Cria um novo grafo transposto com os mesmos vértices
  ...
  for (unsigned int i = 0; i < g->numVertices; i++, ListMoveToNext(vertices)) {
    ...
    for (unsigned int j = 0; j < ListGetSize(edges); j++, ListMoveToNext(edges)) {
      ...
      // Adiciona a aresta invertida no grafo transposto
      if (g->isWeighted) {
        GraphAddWeightedEdge(transposed, edge->adjVertex, vertex->id, edge->weight);
      } else {
        GraphAddEdge(transposed, edge->adjVertex, vertex->id);
      }
    }
  }
